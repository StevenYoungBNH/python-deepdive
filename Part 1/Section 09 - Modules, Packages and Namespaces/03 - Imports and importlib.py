...
# In the last video we saw how we could, in a simplistic manner, mimic Python's
# import.
#
# There is absolutely no need to do this since Python itself provides that
# functionality, both as a built-in function (`import`) and in the standard
# library module `importlib`.
#
# In fact, if you want to see how imports are done in pure Python code you can
# always look at the source code for that library (you should now know where to
# find that on your local machine - you have to first identify a Pythyon
# environment (`sys.exec_prefix`) and then look in the `lib` folder:
...
# You'll find something a little different - `importlib` is not actually a pure
# module (it's still a module type object) - it's actually a package - more on that later.
...
# The problem doing it this way is that **our** module namespace does not have
# a symbol for `fractions` (but it **is** in `sys.modules`):
...
#
# amongst others. Furthermore, Python can reach inside `zip` archives for code
# (as well as other packaged distribution files such as those used by Egg or Wheel).

# In very broad terms the import system, once the "source" code has been located
# works as we saw in the last video.
#
...
#
# The **finders** are responsible for finding the module/package and returning
# the module spec, while the **loaders**, are responsible for "loading" the
# source code that is then used in the final steps to compile, execute and cache
# the module object. An object that implements both is called an **importer** -
# but they are still two separate concepts.

# Python provides a number of standard finders and importers, such as:
#
# * built-in modules
# * frozen modules
# * import path finder (finds source code files on the import path - for example
# the `sys.path` entries we have seen before)
#
# What's interesting about the import path finder and loader is that they can
# search (and load from) zip archives.
#
# In fact it can even be extended to search other resources, including url's,
# databases, etc. You could theoretically store code in a Mongo or Redis
# database and import directly from there!
...
# As you can see the finder determined where the source code was located, and
# also indicated that the loader to be used is the SourceFileLoader.
#
# How does Python know which finder to use in the first place?
#
# It doesn't really - it will go through a bunch of finders, one by one, until
# one returns a module spec - if it exhausts all the registered finders and
# finds nothing, then we get the module not found exception:
...
# When we import our custom file-based modules, the `PathFinder` will be used to
# find the file.
...
# As you can see, it found the file and indicated it would be imported using the
# SourceFileLoader.
...
# Now let's go ahead and write a file somewhere other than our source folder -
# you'll have to change this code to specify your path where you want that module
# file to be created:
...
print(print(module2.x))


# We can "hack" the `sys.path` list by adding our own entries directly - but
# this means we would have to hard code these paths in our code, or potentially
